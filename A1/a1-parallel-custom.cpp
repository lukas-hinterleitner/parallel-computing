...
// introduce a safe cache for worker threads
            ...
            std::atomic<int> &consumed_count, std::vector<std::atomic<int>> &number_counts)
{
    ...
    for (;;) {
        ...
        if (auto search = prime_cache.find(num); search != prime_cache.end()) {
            is_prime = search->second;
        } else {
            is_prime = kernel(num) == 1;
            prime_cache.put(num, is_prime);
        }
