 ...
 *
 */
void print_output(unsigned int num_threads, int primes, int nonprimes, double mean, std::vector<int> number_counts, std::chrono::high_resolution_clock::time_point t1, std::chrono::high_resolution_clock::time_point t2, bool only_exec_times, bool no_exec_times)
{
    if (!only_exec_times)
    {
...
void parse_args(int argc, char **argv, int &num_threads, std::string &filename, bool &no_exec_times, bool &only_exec_times)
{
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        else if (std::string(argv[i]) == "--no-exec-times")
        {
            no_exec_times = true;
        }
        else if (std::string(argv[i]) == "--only-exec-times")
        {
